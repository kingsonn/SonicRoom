import {
  ...
  import bpic from '../Dashboard/components/backg.jpg'
  ...
  async function loadStreamId(){
    ...
    // const lmao = ethers.utils.getAddress()
    
    let contract = new ethers.Contract(vAddress, CreatorProfileAbi, signer)
    let transaction = await contract.getUserProfile(userAddress)
  ...
  // }, [socketClient])
    ...
    return (
      <Card style={{backgroundColor:"black"}} backgroundColor={"black"} margin={0} padding={0} overflowX={{ sm: "scroll", xl: "hidden" }}>
      
        <Suspense style={{backgroundColor:"black"}} fallback={<h1>Loading...</h1>}>
      <Canvas gl={{preserveDrawingBuffer: true}} shadowMap sRGB style={{height:"98vh", width:"98vw",backgroundColor:"black"}}>
    {/* <Sky sunPosition={[100, 20, 100]} /> */}
    <SkyBox/>
    <ambientLight intensity={0.25} />
    <pointLight castShadow intensity={0.7} position={[100, 100, 100]} />
    <Hud position={[0, 0, -2]} />
    <LiveVideo playbackId={playbackId} userAddress={userAddress}/>
  <Recordings userAddress={userAddress}/>
  <Vod userAddress={userAddress}/>
  <Nfts userAddress={userAddress}/>
  <Billboard userAddress={userAddress}/>
  {/* <Plane/> */}
  {/* {Object.keys(clients)
